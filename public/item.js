...
function getRange() {
    ...
    return RANGES.includes(stored ?? '') ? stored : '1w';
}
...
function getCached(id, range) {
    const entry = safeJsonParse(sessionStorage.getItem(`ge_item_${id}_${range}`), null);
...
async function load() {
    ...
    try {
        const { ok, data } = await fetchJson(`/api/ge/item/${id}?range=${range}`);
        if (!ok)
