...
export function render(el, { points = [], range = '1w', series = {} } = {}) {
    ...
    for (let i = 0; i < points.length; i++) {
        if (showHigh && highs[i] != null)
            prices.push(highs[i]);
        if (showLow && lows[i] != null)
            prices.push(lows[i]);
        if (!showHigh && !showLow && (highs[i] ?? lows[i]) != null)
            prices.push((highs[i] ?? lows[i]));
    }
