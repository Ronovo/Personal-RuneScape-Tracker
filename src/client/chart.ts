...
function polylineSegments(coords: (string | null | undefined)[]): string[][] {
  const segs: string[][] = [];
  let cur: string[] = [];
...
export function render(el: HTMLElement, { points = [], range = '1w', series = {} }: RenderOptions = {}): void {
  ...
  const highCoords: (Coord | null)[] = highs.map((v, i) => (v == null ? null : { x: xAt(i), y: yAt(v) }));
  const lowCoords: (Coord | null)[] = lows.map((v, i) => (v == null ? null : { x: xAt(i), y: yAt(v) }));
