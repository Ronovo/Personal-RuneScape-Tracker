...
const { image_name_conversion } = JSON.parse(
  readFileSync(path.join(__dirname, 'image_name_conversion.json'), 'utf8')
);
const { auto: AUTO_NAME_CONVERSION, manual: MANUAL_NAME_CONVERSION } = image_name_conversion;
...
export async function fetchCollectionLog(username) {
  ...
  if (!res.ok) {
    throw Object.assign(new Error('Collection log lookup failed'), { statusCode: 502 });
  }

  const body = await res.json();
  if (body.error) {
    throw Object.assign(
      new Error('No synced collection log found for this player (they may not use the TempleOSRS RuneLite plugin, or the name is misspelled)'),
      { statusCode: 404 }
    );
  }

  const d = body.data;
  const groups = Object.entries(CATEGORY_GROUPS).map(([group, keys]) => ({
    group,
    categories: keys.map((k) => {
      const items = (d.items[k] ?? []).map((i) => ({ ...i, icon: iconUrl(i.name) }));
      return {
        key: k,
        name: prettify(k),
  ...
  const data = {
    username: d.player_name_with_capitalization || d.player,
